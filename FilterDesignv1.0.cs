...
static double[] Filter(double filterM, double[] vFreqs, double[] vMags, out double[] h)
{
    ...
    if (filterM < 1024)
        ...
        npt = Math.Pow(2, Math.Ceiling(Math.Log(filterM) / Math.Log(2)));

    double n_out = Math.Round(filterM);
    ...
    if (Math.Abs(vFreqs[0]) > eps || Math.Abs(vFreqs[nf - 1] - 1) > eps)
    {
    ...
    {
        int ne = (int)Math.Floor(vFreqs[i + 1] * npt);
        if (nb < 0 || ne > npt)
        {
